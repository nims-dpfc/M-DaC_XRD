                ...
                if flag == 0:
                    if line.find('Numeric Data') > -1:
                        ...
                        if key[0].find('SIZE2') > -1:
                            pixel_x = key[1]
                            key = line.split(':')
                        if key[0].find('PXD_DETECTOR_SIZE') > -1:
                            value = key[1].split(' ')
                            x_scale = float(value[1]) / float(pixel_x)
                            y_scale = float(value[0]) / float(pixel_y)
                        if key[0].find('XY_STAGE_UNITS') > -1:
                            value = key[1].split(' ')
                            x_unit = value[1]
                            y_unit = value[0]
                        if key[0].find('PXD_CREATE_DATETIME') > -1:
                            acqdate = line
                            acqdate = acqdate.replace("PXD_CREATE_DATETIME:","")
